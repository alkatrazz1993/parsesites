...
<html lang="en">
    <head>
        ...
        <title>Результаты последней проверки универсального поиска| findmyinfo</title>
    ...
    <body>
        ...
        <div align="center" class="col-sm-12">
                <?if(!empty($info_search_g["Цвет основной границы"][0])){?>
                <div class="revealator-slideup revealator-once">
                    <div id="headerpd" style="border-radius:10px;border:2px solid #0c235a;text-align:center;font-weight:bold;padding:5px;">
                        <p style="margin:0px;" colspan="2">Результаты последней проверки универсального поиска в <span style="color:#00897b;font-weight:bold;">Google</span> <span style="color:#FFAB00"><?=$datausers[0]["last_update"]?></span>:</p>
                    </div> 
                    <?if($info_search_g["ФИО"][0] == "Поиск времено не доступен. Обратитесь к Администрации."){?>
                        <img src="/upload/scan/result_null.png" width="300" height="300">
                    <?}else{?>
                    <?if($info_search_g["Цвет основной границы"][0] == "#5cf442"){?>
                        <img src="/upload/scan/result_green.png" width="300" height="300">
                    <?}else if($info_search_g["Цвет основной границы"][0] == "#FFA500"){?>
                        ...
                        <?$intvul = 1; foreach($vulnerability_g as $vul){foreach($vul as $v){if(!empty($v)){?>
                    ...
                    <div id="pdncontent" style="display: inline-block;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:10px solid <?=$info_search_g["Цвет основной границы"][0]?>;">
                <?}else{?>
                <div class="revealator-slideup revealator-once">
                    <div id="headerpd" style="border-radius:10px;border:2px solid #0c235a;text-align:center;font-weight:bold;padding:5px;">
                        <p style="margin:0px;" colspan="2">Проверок в <span style="color:#00897b;font-weight:bold;">Google</span> еще не было</p>
                    </div>  
                </div><!--slideup-->
                <?}?> 
                    ...
                    $b = 0;
                    if(!empty($info_search_g["Цвет основной границы"][0])){
                            foreach($info_search_g as $key => $value){
                            ...
                            ?>
                                <div style="display: inline-block;" class="revealator-zoomin revealator-delay<?=$i?> revealator-once">
                                    ...
                                    <div align="left" id="border" style="display: inline-block;float:left;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:5px solid <?=$info_search_g["Цвет границы"][$c]?>;">
                                    <?}}?>
                                    <div>
        ...
        <!-- ================================YANDEX==================================-->
            <div align="center" class="col-sm-12">
                <?if(!empty($info_search_y["Цвет основной границы"][0])){?>
                <div class="revealator-slideup revealator-once">
                    <div id="headerpd" style="border-radius:10px;border:2px solid #0c235a;text-align:center;font-weight:bold;padding:5px;">
                        <p style="margin:0px;" colspan="2">Результаты последней проверки универсального поиска в <span style="color:#00897b;font-weight:bold;">Yandex</span> <span style="color:#FFAB00"><?=$datausers[0]["last_update"]?></span>:</p>
                    </div>
                    <?if($info_search_y["ФИО"][0] == "Поиск времено не доступен. Обратитесь к Администрации."){?>
                        <img src="/upload/scan/result_null.png" width="300" height="300">
                    <?}else{?> 
                    <?if($info_search_y["Цвет основной границы"][0] == "#5cf442"){?>
                        <img src="/upload/scan/result_green.png" width="300" height="300">
                    <?}else if($info_search_y["Цвет основной границы"][0] == "#FFA500"){?>
                        <img src="/upload/scan/result_yellow.png" width="300" height="300">
                        <p style="color:#ef0404;text-align:left"><strong>Возможные угрозы:</strong><p>
                        <?$intvul = 1; foreach($vulnerability_y as $vul){foreach($vul as $v){if(!empty($v)){?>
                            <p style="text-align:left;font-weight:bold;"><?=$intvul?>. <?=$v?><p>
                        <?$intvul++;}}}?>
                        <br>
                        <div>
                            <p><strong>Отметьте те ресурсы, где расположена <span style="color:#ef0404">НЕ ВАША</span> информация:</strong></p>
                        </div>
                    <?}else{?>
                        <img src="/upload/scan/result_red.png" width="300" height="300">
                        <div>
                    ...
                    <div id="pdncontent" style="display: inline-block;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:10px solid <?=$info_search_y["Цвет основной границы"][0]?>;">
                <?}else{?>
                    <div class="revealator-slideup revealator-once">
                        <div id="headerpd" style="border-radius:10px;border:2px solid #0c235a;text-align:center;font-weight:bold;padding:5px;">
                            <p style="margin:0px;" colspan="2">Проверок в <span style="color:#00897b;font-weight:bold;">Yandex</span> еще не было</p>
                        </div>  
                    </div><!--slideup-->
                <?}?> 
                    ...
                    <form align="center" id="formresult" action="#" method="post" class="formsignin">
                        <input type="text" value="1" hidden
                            name="SAVE_STATISTIC_YANDEX_FORM">
                    <?$i = 2;
                    $c = 0;
                    $b = 0;
                    if(!empty($info_search_y["Цвет основной границы"][0])){
                            foreach($info_search_y as $key => $value){
                                
                            ?>
                                <div style="display: inline-block;" class="revealator-zoomin revealator-delay<?=$i?> revealator-once">
                                    <?if($value[0] == 'Список ресурсов пуст'){?>
                                        <div align="left" style="display: inline-block;float:left;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:5px solid #5cf442;">
                                    <?} else if($value[0] == 'Нет интернет соединения'){?>
                                        <div align="left" style="display: inline-block;float:left;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:5px solid grey;">
                                    <?}else{ if($key != "Цвет границы" && $key != "Цвет основной границы"){?>
                                    
                                    <div align="left" id="border" style="display: inline-block;float:left;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:5px solid <?=$info_search_y["Цвет границы"][$c]?>;">
                                    <?}}?>
                                    <div>
                                        <h4 style="text-align:center;" colspan="2"><?if($key != "Цвет границы" && $key != "Цвет основной границы"){if($key === 'Мобильный телефон'){  echo "<strong style='color:#795548'>$key: +7"; echo $datausersup[$key] . "</strong>" . "<br>";}else{echo "<strong style='color:#795548'>$key: " . $datausersup[$key] . "</strong>" . "<br>";}}?></h4>                
                                    </div>
