...
<html lang="en">
    <head>
        ...
        <title>Поиск данных в Google и Yandex | findmyinfo</title>
    ...
    <body id="body">
        ...
        <div class="revealator-slideup revealator-once">
            ...
            <div class="revealator-slideup revealator-once">
                <h1 align="center" class="form-signin-heading"><span style="color:#00897b;font-weight:bold;">Универсальный</span> поиск данных</h1>
            ...
            <div align="center" class="col-sm-9">
                <div id="vulng">
                <?if(!empty($vulnerability_g)){if($vulnerability_g[0] != "Угроз нет"){?>
                <p style="color:#ef0404;text-align:left"><strong>Возможные угрозы:</strong><p>
                <?}}?>
                        <?$intvul = 1; foreach($vulnerability_g as $vul)if($vulnerability_g[0] != "Угроз нет"){{foreach($vul as $v){if(!empty($v)){?>
                            <p style="text-align:left;font-weight:bold;"><?=$intvul?>. <?=$v?><p>
                        <?$intvul++;}}}}?>
                        </div><!--vuln_g -->
                <?if(!empty($info_search)){?>
                <div class="revealator-slideup revealator-once">
                    <div id="headerpdg" style="border-radius:10px;border:2px solid #0c235a;text-align:center;font-weight:bold;padding:5px;">
                        <p style="margin:0px;" colspan="2">Найденные ресурсы в <span style="color:#00897b;font-weight:bold;">Google</span>:</p>
                    </div> 
                </div>
                    <div id="pdncontentg" align="left" style="display: inline-block;float:left;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:10px solid <?=$color_top_border_g?>;">
                <?}else{?>
                <div class="revealator-slideup revealator-once">
                    <div id="headerpdg" style="border-radius:10px;border:2px solid #0c235a;text-align:center;font-weight:bold;padding:5px;">
                        <p style="margin:0px;" colspan="2">Здесь Вы увидите не сохраненные результаты проверки в <span style="color:#00897b;font-weight:bold;">Google</span></p>
                    </div>  
                </div>
                <?}?> 
                    ...
                    if(!empty($info_search)){
                        foreach($info_search as $key => $param)
				        {
                            ...
                            if($googleSearch[$key][0] == 'Список ресурсов пуст'){
                            ?>
                            ...
                            <?} else if($googleSearch[$key][0] == 'Поиск времено не доступен. Обратитесь к Администрации.'){?>
                            ...
                            <?}else{?>
                            <div align="left" id="border" style="display: inline-block;float:left;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:5px solid <?=$color_border_g[$key]?>;">
                            <?}?>
                                <div>
                                    ...
                                    <?if(!empty($googleSearch[$key])){if($googleSearch[$key][0] != 'Список ресурсов пуст' && $googleSearch[$key][0] != 'Поиск времено не доступен. Обратитесь к Администрации.'){
                                    ?><p>Найдено ресурсов: <span id="resourseTotal"><?=count($googleSearch[$key])?></span></p><?}else {
                                        ?><p>Найдено ресурсов: <span id="resourseTotal">0</span></p><?
                                    }}?>
                                </div>
                                <?
                                if(!empty($googleSearch[$key])){
                                    foreach($googleSearch[$key] as $link){
        ...
        <br>
                ...
                <p style="color:#ef0404;text-align:left"><strong>Возможные угрозы:</strong><p>
                <?}}?>
                        <?$intvul = 1; foreach($vulnerability_y as $vul)if($vulnerability_y[0] != "Угроз нет"){{foreach($vul as $v){if(!empty($v)){?>
                            <p style="text-align:left;font-weight:bold;"><?=$intvul?>. <?=$v?><p>
                        <?$intvul++;}}}}?>
                       </div><!--vuln_y -->
                <?if(!empty($info_search)){?>
                <div class="revealator-slideup revealator-once">
                    <div id="headerpdy" style="border-radius:10px;border:2px solid #0c235a;text-align:center;font-weight:bold;padding:5px;">
                        <p style="margin:0px;" colspan="2">Найденные ресурсы в <span style="color:#00897b;font-weight:bold;">Yandex</span>:</p>
                    </div> 
                </div>
                    <div id="pdncontenty" align="left" style="display: inline-block;float:left;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:10px solid <?=$color_top_border_y?>;">
                <?}else{?>
                <div class="revealator-slideup revealator-once">
                    <div id="headerpdy" style="border-radius:10px;border:2px solid #0c235a;text-align:center;font-weight:bold;padding:5px;">
                        <p style="margin:0px;" colspan="2">Здесь Вы увидите не сохраненные результаты проверки в <span style="color:#00897b;font-weight:bold;">Yandex</span></p>
                    </div>  
                </div>
                <?}?> 
                    ...
                    if(!empty($info_search)){
				        ...
				        {
                            ?><div style="display: inline-block;" class="revealator-slideup revealator-delay<?=$i?> revealator-once"><?
                            if($yandexSearch[$key][0] == 'Список ресурсов пуст'){
                            ?>
                                <div align="left" style="display: inline-block;float:left;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:5px solid #5cf442;">
                            <?} else if($yandexSearch[$key][0] == 'Поиск времено не доступен. Обратитесь к Администрации.'){?>
                                <div align="left" style="display: inline-block;float:left;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:5px solid grey;">
                            <?}else{?>
                            <div align="left" id="border" style="display: inline-block;float:left;padding:10px;margin:10px;border-radius:10px;border-right:2px solid #0c235a;border-bottom:2px solid #0c235a;border-top:2px solid #0c235a;border-left:5px solid <?=$color_border_y[$key]?>;">
                            <?}?>
                                <div>
                                    <h4 style="text-align:center;" colspan="2"><?if(!empty($param)){if($key === 'Мобильный телефон'){?><?echo "<strong style='color:#795548'>$key: " . "+7" . $param . "</strong>" . "<br>";?><?}else{echo "<strong style='color:#795548'>$key: " . $param . "</strong>" . "<br>";}}else{echo "<strong style='color:#00897b'>Поиск: $key</strong>";}?></h4>
                                    <?if(!empty($yandexSearch[$key])){if($yandexSearch[$key][0] != 'Список ресурсов пуст' && $yandexSearch[$key][0] != 'Поиск времено не доступен. Обратитесь к Администрации.'){
                                    ?><p>Найдено ресурсов: <span id="resourseTotal"><?=count($yandexSearch[$key])?></span></p><?}else {
                                        ?><p>Найдено ресурсов: <span id="resourseTotal">0</span></p><?
                                    }}?>
                                </div>
                                <?
                                if(!empty($yandexSearch[$key])){
                                    foreach($yandexSearch[$key] as $link){
                                        if($link == "Список ресурсов пуст"){?>
                                    <div>
                                        <p style="border-bottom:1px dotted black;color:#5cf442;font-weight:bold;"><?=$link?>&nbsp; 
                                            <span style="float:right;"><i style="color:#5cf442" class="fas fa-check-circle"></i></span>
